...
public class ServiceListTopbar extends MultiElementContainer<ServiceScreen> {
    ...
    public void init() {
        ...
        addButton(new Button(x + w * 10 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new ChatComponentTranslation("jsmacros.enabled"), (btn) -> {
            //TODO: sort by enabled
            parent.reload();
        }));

        addButton(new Button(x + w * 11 / 12 + 1, y + 1, w / 12 - 1, height - 3, textRenderer, 0, 0xFF000000, 0x7F7F7F7F, 0xFFFFFF, new ChatComponentTranslation("jsmacros.running"), (btn) -> {
